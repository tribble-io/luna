  ...
  componentDidMount() {
    setTimeout(() => {
      ...
      if (this.state.checkFilter) {
        ...
        fetch(
          `http://theatre.restomatik.ru:1337/api/shows?filters[date][$gte]=${calDate}&filters[date][$lt]=${calLastDate}&sort[0]=date&${filtPathc}populate=play.scene&filters[play][title][$containsi]=${seachEl}`
        )
          .then((res) => res.json())
          .then((result) => {
